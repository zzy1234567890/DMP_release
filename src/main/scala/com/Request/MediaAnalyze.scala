...
//        val Array(inputPath)=args
    ...
    val map: collection.Map[String, String] = spark.sparkContext.textFile("E://sparkNewStage/test/dataP/app_dict.txt").map(_.split("\t", -1))
    ...
    val tup: RDD[(String, List[Double])] = df.rdd.map(row => {
      ...
      val medtype = if (!appname.isEmpty) appname else broadcastInfo.value.getOrElse(appid, "未知媒体")
